...
> **⚠️ Nieaktualne od 2026-07-17:** klient zaakceptował migrację VPS z Mikr.us na Sferahost (powtarzające się awarie dysku I/O, szczegóły i uzasadnienie w `post-mortem/vps-migration-decision.md`). Model sieciowy poniżej (IPv6 + Cloudflare AAAA) zakładał, że domena klienta jest proxowana przez Cloudflare jak `giemza.dev` — ale `coaction.pl` jest zarządzana przez panel Sferahost, bez proxy. **Potwierdzone (2026-07-17): nowy VPS ma IPv4** — model docelowy to zwykły rekord **A** dla `coaction.pl` w panelu Sferahost, Caddy nadal robi automatic HTTPS na 80/443, bez zależności od IPv6/Cloudflare. Punkty 1–4 poniżej opisują stary (Mikr.us) model i wymagają aktualizacji po migracji, ale kierunek zmiany (A record zamiast AAAA) jest już ustalony.
