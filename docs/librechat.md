...
**Gotcha:** `librechat.yaml` konfiguruje jeden custom endpoint (OpenRouter), reużywając istniejący `OPENROUTER_API_KEY` (ten sam, którego używa n8n) zamiast trzymać drugi sekret. LibreChat czyta klucz endpointu z env var o dokładnie tej nazwie, jaka jest w yamlu — stąd rebind we fragmentach compose (`fragments/librechat-compose.yml`): `OPENROUTER_KEY=${OPENROUTER_API_KEY}`. Nazwanie tego wprost `OPENROUTER_API_KEY` przekierowałoby też wbudowany endpoint OpenAI LibreChata przez OpenRouter, czego nie chcemy.
