...
Kod (serwisy `minio`/`minio-init`, wpis w Caddyfile, `docker/minio-init.sh`) jest już w repo (`Add MinIO for NocoDB attachments/avatars`). Na VPS trzeba go wdrożyć i dopiąć konfigurację, która nie idzie z Gitem: `.env` i DNS.
...
- `NOCODB_MINIO_ACCESS_KEY`, `NOCODB_MINIO_SECRET_KEY` — dane roli, którą `minio-init.sh` utworzy automatycznie (nie ręcznie w MinIO)
...
`Makefile` używa `docker compose -f docker-compose.yml -f docker-compose.prod.yml`. To ściągnie `minio` + `minio/mc`, wystartuje `minio`, poczeka na healthcheck (`mc ready local`) i uruchomi jednorazowo `minio-init` — tworzy buckety z PRD §8.5 (`offers`/`templates` versioned, `recordings`/`backups` z lifecycle expiry) oraz least-privilege rolę `nocodb` scoped do bucketu `attachments`.

## 5. Weryfikacja

```bash
make ps
docker compose -f docker-compose.yml -f docker-compose.prod.yml logs minio-init
```
